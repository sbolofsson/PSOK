...
namespace InterLinq.Communication.Remoting
{
    ...
    public class ClientQueryRemotingHandler : ClientQueryHandler
    {
        ...
        private readonly bool _makeDefaultConnection;
        private string _url;
        ...
        {
            _makeDefaultConnection = makeDefaultConnection;
        }
        ...
        public override void Connect()
        {
            if (QueryRemoteHandlerInstance == null)
            {
                if (_makeDefaultConnection)
                {
                    ...
                    _url = string.Format("{0}://{1}:{2}/{3}", RemotingConstants.DefaultServiceProtcol, RemotingConstants.DefaultServerName, RemotingConstants.DefaultServicePort, RemotingConstants.DefaultServerObjectName);
                }

                QueryRemoteHandlerInstance = (IQueryRemoteHandler)Activator.GetObject(typeof(IQueryRemoteHandler), _url);
